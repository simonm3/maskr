...
from rpn.gen_targets import build_rpn_targets
import random
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        images = utils.images.mold_image(image.astype(np.float32), self.config)
    ...
    def load_image_gt(self, config, image_id, augment=False,
        ...
        shape = image.shape
        image, window, scale, padding = utils.images.resize_image(
            image,
            min_dim=config.IMAGE_MIN_DIM,
            max_dim=config.IMAGE_MAX_DIM,
            padding=config.IMAGE_PADDING)
        mask = utils.images.resize_mask(mask, scale, padding)
        ...
        bbox = utils.boxes.extract_bboxes(mask)
        ...
        if use_mini_mask:
            mask = utils.images.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)

        # Image meta data
        image_meta = utils.images.compose_image_meta(image_id, shape, window, active_class_ids)
